 ...
 class RecentList extends Component{
    ...
    constructor(props){
        ...
        this.state={
            ...
            origin_recentItems:JSON.parse(localStorage.getItem('recentItems_sample')),
            check : false, 
        }
    }
    ...
    selectBrand = (res) => {
        ...
        if(this.state.brandName.includes(res)){
            ...
            this.setState({brandName: this.state.brandName},()=>{this.filterData()})
        }
        // 최초 선택 => 필터 기능
        else{
            
            this.setState({brandName:this.state.brandName.concat(res)},()=>{this.filterData()})
        }
    ...
    // 관심없음 체크박스 
    checkClick =(res)=>{
        this.setState({check:res}, ()=>{this.state.check ? this.setDislikeFilter():this.setDefaultFilter()})
    ...
    setDislikeFilter(){
        ...
        if(this.state.brandName.length>0){
            for(recentData of this.state.recentItems){
                for(dislikeData of this.state.dislikeItems){
                    if(dislikeData.title === recentData.title && dislikeData.price === recentData.price && dislikeData.brand === recentData.brand){
        ...
        resultList = (this.state.recentItems.filter(item => !findTitle.includes(item.title)))
    ...
    render(){
        return(
            <div style={{width:'650px', flexDirection:'column', margin:'auto'}}>
                <Filter brand={this.selectBrand} setCheck={this.checkClick} check={this.state.check}/>
                <Item selectBrand={this.state.brandName} productData={this.state.recentItems}/>
